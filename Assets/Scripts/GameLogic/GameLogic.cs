    ...
    public class GameLogic
    {
        ...
        public virtual void StartGame()
        {
            ...
            foreach (Player player in game_data.players)
            {
                ...
                //Hp / mana
                player.hp_max = pdeck != null ? pdeck.startHp : GameplayData.Get().hpStart;
                player.hp = player.hp_max;
                player.mana_max = pdeck != null ? pdeck.startMana : GameplayData.Get().manaStart;
                ...
                bool is_random = level == null || level.firstPlayer == LevelFirst.Random;
                if (is_random && player.player_id != game_data.first_player && GameplayData.Get().secondBonus != null)
                {
                    Card card = Card.Create(GameplayData.Get().secondBonus, VariantData.GetDefault(), player);
        ...
        public virtual void StartTurn()
        {
            ...
            {
                DrawCard(player, GameplayData.Get().cardsPerTurn);
            }

            //Mana 
            player.mana_max += GameplayData.Get().manaPerTurn;
            player.mana_max = Mathf.Min(player.mana_max, GameplayData.Get().manaMax);
            player.mana = player.mana_max;

            //Turn timer and history
            game_data.turn_timer = GameplayData.Get().turnDuration;
        ...
        public virtual void SetPlayerDeck(Player player, DeckData deck)
        {
            ...
            if (puzzle != null)
            {
                foreach (DeckCardSlot card in puzzle.boardCards)
                {
        ...
        public virtual void DrawCard(Player player, int nb = 1)
        {
            for (int i = 0; i < nb; i++)
            {
                if (player.cards_deck.Count > 0 && player.cards_hand.Count < GameplayData.Get().cardsMax)
                {
        ...
        protected virtual void AfterAbilityResolved(AbilityData iability, Card caster)
        {
            ...
            {
                foreach (AbilityData chain_ability in iability.chainAbilities)
                {
