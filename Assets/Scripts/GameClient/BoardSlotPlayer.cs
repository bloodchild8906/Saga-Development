    ...
    public class BoardSlotPlayer : BSlot
    {
        ...
        private void OnAbilityStart(AbilityData iability, Card caster)
        {
            if (iability != null && caster != null)
            {
                ...
                {
                    FXTool.DoFX(iability.casterFX, transform.position);
                    AudioTool.Get().PlaySFX("fx", iability.castAudio);
                }
            }
        }
        ...
        private void OnAbilityEffect(AbilityData iability, Card caster, Player target)
        {
            if (iability != null && caster != null && target != null)
            {
                ...
                {
                    FXTool.DoFX(iability.targetFX, transform.position);
                    FXTool.DoProjectileFX(iability.projectileFX, GetFXSource(caster), transform, iability.GetDamage());
                    AudioTool.Get().PlaySFX("fx", iability.targetAudio);
                }
            }
        }
