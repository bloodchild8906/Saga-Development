...
namespace TcgEngine.Client
{
    ...
    public class GameBoard : MonoBehaviour
    {
        ...
        private IEnumerator EndGameRun()
        {
            ...
            if (win)
                AudioTool.Get().PlaySFX("ending_sfx", AssetData.Get().winAudio);
            else
                AudioTool.Get().PlaySFX("ending_sfx", AssetData.Get().defeatAudio);

            if (win)
                AudioTool.Get().PlayMusic("music", AssetData.Get().winMusic, 0.4f, false);
            else
                AudioTool.Get().PlayMusic("music", AssetData.Get().defeatMusic, 0.4f, false);
