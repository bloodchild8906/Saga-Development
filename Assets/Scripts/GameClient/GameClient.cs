    ...
    public class GameClient : MonoBehaviour
    {
        ...
        public virtual void ConnectToAPI()
        {
            ...
            if (!Authenticator.Get().IsSignedIn())
            {
                ...
                {
                    player_settings.deck = new UserDeckData(GameplayData.Get().testDeck);
                }

                if (!ai_settings.HasDeck())
                {
                    ai_settings.deck = new UserDeckData(GameplayData.Get().testDeckAI);
                    ai_settings.ai_level = GameplayData.Get().aiLevel;
                }
            }
