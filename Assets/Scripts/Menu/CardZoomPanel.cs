    ...
    public class CardZoomPanel : UIPanel
    {
        ...
        protected override void Update()
        {
            ...
            if (card != null)
            {
                ...
                sell_cost.text = Mathf.RoundToInt(cost * GameplayData.Get().sellRatio).ToString();
            }
        }
        ...
        {
            int quantity = GetBuyQuantity();
            int cost = Mathf.RoundToInt(quantity * card.cost * variant.costFactor * GameplayData.Get().sellRatio);
            if (quantity <= 0)
                return;
