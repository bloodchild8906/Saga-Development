...
namespace TcgEngine.Server
{
    ...
    public class GameServer
    {
        ...
        {
            //Setup AI
            bool ai_vs_ai = !is_dedicated_server && GameplayData.Get().aiVSAI;
            foreach (Player player in game_data.players)
            {
                if (player.is_ai || ai_vs_ai)
                {
                    AIPlayer ai_gameplay = AIPlayer.Create(GameplayData.Get().aiType, gameplay, player.player_id, player.ai_level);
